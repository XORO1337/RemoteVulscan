...
export default function HomePage() {
  ...
  // ---------------- Effects --------------------------------------------------
  useEffect(() => {
  // fetch public config (served by frontend itself)
  ...
  async function fetchScans() {
    try {
      ...
      // backend returns { scans, pagination } structure
      const data: Scan[] = Array.isArray(payload) ? payload : (payload.scans || []);
      setScans(data)
      if (currentScan) {
  ...
  async function fetchScanDetails(id: string) {
    ...
    const data: Scan = (dataWrapped.scan) ? dataWrapped.scan : dataWrapped
  ...
  async function startScan() {
    ...
    try {
      ...
      const scan: Scan = resp.scan || resp; 
      setCurrentScan(scan)
  ...
  function startPolling(id: string) {
    ...
    pollRef.current = setInterval(async () => {
      ...
      const scan: Scan = wrapped.scan || wrapped;
      setCurrentScan(scan)
